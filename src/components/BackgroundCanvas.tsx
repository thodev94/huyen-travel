...
const BackgroundCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // BUILDINGS (No Borders, Overlap Blur/Shadow & Dynamic Colors)
      let bkgC = '#1c3d6e'; 
      if (p < 0.25) { 
      ...
      sceneData.buildings.forEach(b => {
        ...
        if (!isN) {
          ...
          ctx.beginPath(); ctx.moveTo(x, h - 80); ctx.lineTo(x + b.w, h - 80);
          ctx.lineTo(x + b.w - shadLen, h - 20); ctx.lineTo(x - shadLen, h - 20);
        ...
        ctx.save();
        ctx.fillStyle = bkgC;
        // Shadow Blur for Overlap separation
        ...
        if (b.style === 'flat' || b.style === 'landmark') {
          if (b.style === 'flat') {
            ...
            if (!isN) {
              ...
              if (sX < x + b.w / 2) ctx.fillRect(x, h - b.h, hWidth, b.h); else ctx.fillRect(x + b.w - hWidth, h - b.h, hWidth, b.h);
              ctx.globalAlpha = 1.0;
              ctx.fillStyle = '#111'; ctx.fillRect(x + b.w * 0.4, h - 120, b.w * 0.2, 20); 
            }
          } else {
            let ch=b.h, cw=b.w, cx=x;
            for(let i=0; i<6; i++){ 
              ctx.fillStyle = bkgC; ctx.fillRect(cx, h-ch, cw, ch); 
              if (!isN) {
                ctx.fillStyle = reflectC; ctx.globalAlpha = 0.1;
                if (sX < cx + cw / 2) ctx.fillRect(cx, h - ch, cw * 0.15, ch); else ctx.fillRect(cx + cw * 0.85, h - ch, cw * 0.15, ch);
                ctx.globalAlpha = 1.0;
                if (i === 0) { ctx.fillStyle = '#111'; ctx.fillRect(cx + cw * 0.4, h - 120, cw * 0.2, 20); }
              }
              ch+=b.h*0.08; cw*=0.78; cx+=(b.w-cw)/2; 
            }
          }
          ctx.restore();
          ...
          b.windows.forEach(win => {
            ctx.fillRect(x + win.x, h - b.h + win.y, 2.5, 4.5);
          });
          ctx.shadowBlur = 0;
        } else if (b.style === 'bitexco') {
          ctx.beginPath(); ctx.moveTo(x, h); ctx.lineTo(x+b.w*0.4, h-b.h); ctx.lineTo(x+b.w*0.6, h-b.h); ctx.lineTo(x+b.w, h); ctx.fill();
          if (!isN) {
            ctx.save(); ctx.beginPath();
            if (sX < x + b.w / 2) { ctx.moveTo(x, h); ctx.lineTo(x+b.w*0.4, h-b.h); ctx.lineTo(x+b.w*0.5, h-b.h); ctx.lineTo(x+b.w*0.5, h); }
            else { ctx.moveTo(x+b.w, h); ctx.lineTo(x+b.w*0.6, h-b.h); ctx.lineTo(x+b.w*0.5, h-b.h); ctx.lineTo(x+b.w*0.5, h); }
      ...
      vArr.forEach((v, i) => {
        v.x = (v.x + v.s) % (w + 120);
        ctx.fillStyle = v.color; ctx.fillRect(v.x - 60, h - 65 + (i % 2) * 30, v.w, v.h);
        if (isN) { ctx.fillStyle = '#fff'; ctx.fillRect(v.x - 5, h - 65 + (i % 2) * 30 + 3, 10, 4); }
      });
